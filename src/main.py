...
def main():
    ...
    while True:
        ...
        legal = board.legal_moves(board.black if black_to_move else board.white,
                                  board.white if black_to_move else board.black)
        if legal == 0:
            print(f"{player} has no moves. Pass.")
            black_to_move = not black_to_move
            if board.legal_moves(board.black if black_to_move else board.white,
                                 board.white if black_to_move else board.black) == 0:
